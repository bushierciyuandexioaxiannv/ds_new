...
class Type extends Model {
    ...
    public function getTypebrandList($condition, $field = '*') {
        return db('type_brand')->field($field)->where($condition)->select();
    }
    ...
    public function getSpecByType($where, $field, $order = 's.sp_sort asc, s.sp_id asc') {
        ...
        $result = db('type_spec')->alias('t')->field($field)->where($where)->join('__SPEC__ s', 't.sp_id = s.sp_id')->order($order)->select();
        return $result;
    }
    ...
    public function getTypebrandCount($condition) {
        return db('type_brand')->where($condition)->count();
    }
    ...
    public function typeRelatedJoinList($condition, $type = '', $field = '*', $order = '') {
        ...
        switch ($type) {
            case 'spec':
                ...
                $result = db('type_spec')->alias('type_spec')->field($field)->join('__SPEC__ spec', 'type_spec.sp_id = spec.sp_id')->where($condition)->order($order)->select();
                break;
            case 'attr':
            ...
            case 'brand':
                ...
                $result = db('type_brand')->alias('type_brand')->join('__BRAND__ brand', 'type_brand.brand_id=brand.brand_id')->where($condition)->order($order)->select();
                break;
        }
        return $result;
    }
    ...
    public function addTypebrand($data){
        return db('type_brand')->insertAll($data);
    }
    ...
    public function addTypespec($data){
        return db('type_spec')->insertAll($data);
    }
    ...
    public function delTypebrand($condition){
        return db('type_brand')->where($condition)->delete();
    }
    ...
    public function delTypespec($condition){
        return db('type_spec')->where($condition)->delete();
    }
