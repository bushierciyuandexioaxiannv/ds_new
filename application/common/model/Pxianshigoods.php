...
class Pxianshigoods extends Model {
    ...
    public function getXianshigoodsList($condition, $page = null, $order = '', $field = '*', $limit = 0) {
        if ($page) {
            $res = db('p_xianshi_goods')->field($field)->where($condition)->order($order)->paginate($page,false,['query' => request()->param()]);
            $this->page_info = $res;
            return $res->items();
        } else {
            return $xianshigoods_list = db('p_xianshi_goods')->field($field)->where($condition)->limit($limit)->order($order)->select();
        }
    }
    ...
    public function getXianshigoodsInfo($condition) {
        $result = db('p_xianshi_goods')->where($condition)->find();
        return $result;
    }
    ...
    public function getXianshigoodsInfoByID($xianshigoods_id, $store_id = 0) {
        ...
        $condition = array();
        $condition['xianshi_goods_id'] = $xianshigoods_id;
    ...
    public function addXianshigoods($xianshigoods_info) {
        $xianshigoods_info['state'] = self::XIANSHI_GOODS_STATE_NORMAL;
        $xianshigoods_id = db('p_xianshi_goods')->insertGetId($xianshigoods_info);

        // 删除商品限时折扣缓存
        $this->_dGoodsXianshiCache($xianshigoods_info['goods_id']);

        $xianshigoods_info['xianshi_goods_id'] = $xianshigoods_id;
    ...
    public function editXianshigoods($update, $condition) {
        $result = db('p_xianshi_goods')->where($condition)->update($update);
        if ($result) {
    ...
    public function delXianshigoods($condition) {
        $xianshigoods_list = $this->getXianshigoodsList($condition, null, '', 'goods_id');
        $result = db('p_xianshi_goods')->where($condition)->delete();
        if ($result) {
            if (!empty($xianshigoods_list)) {
                foreach ($xianshigoods_list as $val) {
                    // 删除商品限时折扣缓存
    ...
    public function getXianshigoodsInfoByGoodsID($goods_id) {
        ...
        if (empty($info)) {
            ...
            $xianshigoods_list = $this->getXianshigoodsExtendList($condition, null, 'start_time asc', '*', 1);
        ...
        if (!empty($xianshigoods_info) && ($xianshigoods_info['start_time'] > TIMESTAMP || $xianshigoods_info['end_time'] < TIMESTAMP)) {
    ...
    private function _getXianshigoodsListByGoods($goods_id_string) {
        ...
        $xianshigoods_list = $this->getXianshigoodsExtendList($condition, null, 'xianshi_goods_id desc', '*');
        return $xianshigoods_list;
    }
