...
class Storemsgsetting extends Model
{
    ...
    public function getStoremsgsettingList($condition, $field = '*', $key = '', $page = 0, $order = 'smt_code asc') {
        $res=db('store_msg_setting')->field($field)->where($condition)->order($order)->paginate($page,false,['query' => request()->param()]);
        $this->page_info=$res;
        $result= $res->items();
    ...
    public function getStoremsgsettingInfo($condition, $field = '*') {
        return db('store_msg_setting')->field($field)->where($condition)->find();
    }
    ...
    public function addStoremsgsetting($data) {
        return db('store_msg_setting')->insert($data);
    }
    ...
    public function editStoremsgsetting($data, $condition) {
        return db('store_msg_setting')->where($condition)->update($data);
    }
}