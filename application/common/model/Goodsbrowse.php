...
class Goodsbrowse extends Model {
    ...
    public function getViewedGoodsList($member_id = 0, $shownum = 0) {
        ...
        if ($browselist) {
            $goods_list_tmp = model('goods')->getGoodsList(array('goods_id' => array('in', array_keys($browselist))), 'goods_id, goods_name, goods_jingle, goods_promotion_price, goods_promotion_type, goods_marketprice, goods_image, store_id, gc_id, gc_id_1, gc_id_2, gc_id_3');
            $goods_list = array();
            ...
            foreach ((array) $browselist as $k => $v) {
                if (isset($goods_list[$k])) {
                    $browselist_new[] = array(
                        ...
                        "goods_jingle" => $goods_list[$k]['goods_jingle'],
