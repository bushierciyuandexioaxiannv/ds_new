...
class Goods extends Model {
    ...
    public function addGoodsCommon($data) {
        return db('goods_common')->insertGetId($data);
    }
    ...
    public function getGoodsCommonList($condition, $field = '*', $page = 10, $order = 'goods_commonid desc') {
        $condition = $this->_getRecursiveClass($condition);
        if ($page) {
            $result = db('goods_common')->field($field)->where($condition)->order($order)->paginate($page,false,['query' => request()->param()]);
            $this->page_info = $result;
            return $result->items();
        } else {
            return db('goods_common')->field($field)->where($condition)->order($order)->select();
        }
    }
    ...
    public function getGoodsCommendList($store_id, $limit = 5) {
        $goods_commend_list = $this->getGoodsOnlineList(array('store_id' => $store_id, 'goods_commend' => 1), 'goods_id,goods_name,goods_jingle,goods_image,store_id,goods_promotion_price,goods_price', 0, '', $limit, 'goods_commonid');
    ...
    public function editGoodsPromotionPrice($condition) {
        ...
        foreach ($goods_array as $key => $val) {
            ...
            foreach ($val as $k => $v) {
                // 查询限时折扣时候进行
                $xianshigoods = $pxianshigoods_model->getXianshigoodsInfo(array('goods_id' => $k, 'start_time' => array('lt', TIMESTAMP), 'end_time' => array('gt', TIMESTAMP)));
    ...
    public function editGoodsCommonById($update, $commonid_array) {
        ...
        $result = db('goods_common')->where($condition)->update($update);
        if ($result) {
    ...
    public function getGoodeCommonInfo($condition, $field = '*') {
        return db('goods_common')->field($field)->where($condition)->find();
    }
    ...
    public function getGoodsCommonCount($condition) {
        return db('goods_common')->where($condition)->count();
    }
    ...
    public function delGoodsAll($condition) {
        ...
        // 删除商品公共表数据
        db('goods_common')->where(array('goods_commonid' => array('in', $commonid_array)))->delete();
        // 删除商品图片表数据
