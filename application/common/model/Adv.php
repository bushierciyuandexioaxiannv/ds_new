...
class Adv extends Model {
    ...
    public function addAdvposition($data) {
        return db('adv_position')->insertGetId($data);
    }
    ...
    public function delAdvposition($ap_id) {
        $apId = (int) $ap_id;
        dkcache("adv/{$apId}");
        return db('adv_position')->where('ap_id', $apId)->delete();
    }
    ...
    public function getAdvpositionList($condition = array(), $page = '', $orderby = 'ap_id desc') {
        if ($page) {
            $result = db('adv_position')->where($condition)->order($orderby)->paginate($page, false, ['query' => request()->param()]);
            $this->page_info = $result;
            return $result->items();
        } else {
            return db('adv_position')->where($condition)->order($orderby)->select();
        }
    }

    public function getOneAdvposition($condition = array()) {
        return db('adv_position')->where($condition)->find();
    }
    ...
    public function mbadvlist($condition,$orderby='slide_sort desc'){
         return db('adv')->alias('a')->join('__ADV_POSITION__ n','a.ap_id=n.ap_id')->where($condition)->order($orderby)->select();
    }
    ...
    public function editAdvposition($data) {
        $apId = (int) $data['ap_id'];
        dkcache("adv/{$apId}");
        return db('adv_position')->where('ap_id', $data['ap_id'])->update($data);
    }
